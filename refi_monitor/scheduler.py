...
from .notifications import send_alert_notification, send_all_monthly_reports
...
def scheduled_monthly_report():
    """
    Scheduled task function that sends monthly reports to all users.
    ...
    with app.app_context():
        ...
        try:
            results = send_all_monthly_reports()
            logger.info(
                f"Monthly reports complete: "
                f"sent={results['sent']}, "
...
def init_scheduler(app: Flask):
    ...
    scheduler.add_job(
        ...
        id='monthly_report',
        name='Send monthly reports on 1st of each month',
        replace_existing=True
    )
